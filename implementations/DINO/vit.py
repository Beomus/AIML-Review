...
class MlpHead(nn.Module):
    """
    Simple MLP, connected to the CLS token embedding, with the last layer being
    normalized in a particular way
    ...
    bottleneck_dim: int
        Dimensionality of the second to last layer.

    n_layers: int
        The number of layers.

    norm_last_layer: bool
        If True, freezing the norm of the weight of the last linear layer to 1.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = F.normalize(x, dim=-1, p=2)  # (n_samples, bottleneck_dim)
