...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        q, k, v = rearrange(qkv, 'b n (qkv h d) -> qkv b h n d', qkv=3, h=h)  # split into multihead attentions

        dots = torch.einsum('bhid,bhjd->bhij', q, k) * self.scale  # scale to avoid gradient issues

        if mask is not None:
            mask = F.pad(mask.flatten(1), (1, 0), value=True)
            assert mask.shpape[-1] == dots.shape[-1], f'Mask has incorrect dimension (dots dim {dots.shape[-1]})'
        ...
        out = rearrange(out, 'b h n d -> b n (h d)')  # concat heads into one maxtrix read for next encoder
        out = self.nn1(out)
        out = self.do1(out)
...
class ViT(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, channels=3, dropout=0.1, emb_dropout=0.1):
        super().__init__()
        assert image_size % patch_size == 0, f"Image dimensions ({image_size}) must be divisible by patch size ({patch_size})"
        ...
        self.patch_conv = nn.Conv2d(3, dim, patch_size, stride=patch_size)  # equivalent to x matmul E, with E = embedded matrix
        ...
        self.nn1 = nn.Linear(dim, num_classes)  # if finetuning, just use a linear layer without futher hidden layers
        torch.nn.init.xavier_uniform_(self.nn1.weight)
        torch.nn.init.normal_(self.nn1.bias, std=1e-6)
...
def main():
    ...
    train_dataset = datasets.CIFAR100(DL_PATH, train=True, download=True, transform=transform)
    test_dataset = datasets.CIFAR100(DL_PATH, train=False, download=True, transform=transform)
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=BATCH_SIZE_TRAIN, shuffle=True)
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=BATCH_SIZE_TEST, shuffle=False)

    model = ViT(image_size=32, patch_size=8, num_classes=100, channels=1, dim=1024, depth=6, heads=16, mlp_dim=1024)
